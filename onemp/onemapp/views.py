...
def post_list(request):
    """ —писок всех постов с возможностью сортировки и поиска по title, можно добавить пост"""
    if user and user.is_authenticated:
        posts = Post.objects.filter(author=user)  # получить посты пользовател€
    else:
...
def post_detail(request, post_id):
    """ ѕост с комментари€ми, можно добавить комментарий"""
